...
# La red de estudio es sintética. 
using JuMP
using HiGHS
#using Ipopt
using Plots
...
NOMBRE_ARCHIVO_DATOS ="Predespacho/dat_UC_Ex6.xlsx"; 
TIPO_SOLVER         = 1; # 1 = HiGHS, 2 = Gurobi, 3= Ipopt ; 4 = Pajarito
TIPO_SERIE_DEMANDA  = 1; #1: CON VARIABILIDAD,0: SIN VARIABLIDAD
TIPO_SERIE_ERV      = 1; #1: PERFIL EOLICO pwE, 2 pw5, 3: pw5
sB                  = 100; 

datGen      =   XLSX.readdata(NOMBRE_ARCHIVO_DATOS,"Gen!C4:O8")
datDem      =   XLSX.readdata(NOMBRE_ARCHIVO_DATOS,"dem!A4:D27")
datLineas   =   XLSX.readdata(NOMBRE_ARCHIVO_DATOS,"Lineas!A4:E6")

function fcn_UC_model(;datGen,datDem,datLineas,NOMBRE_ARCHIVO_DATOS,TIPO_SOLVER=TIPO_SOLVER, TIPO_SERIE_DEMANDA=TIPO_SERIE_DEMANDA, TIPO_SERIE_ERV=TIPO_SERIE_ERV)
    ################################################################
    # LEE DATOS 
    ################################################################
    # Datos red
    ...
    if TIPO_SERIE_ERV==1
        datEr       =   XLSX.readdata(NOMBRE_ARCHIVO_DATOS,"ERV!J6:J29")

    elseif TIPO_SERIE_ERV==2
        datEr       =   XLSX.readdata(NOMBRE_ARCHIVO_DATOS,"ERV!H6:H29") # pw5

    elseif TIPO_SERIE_ERV==3
        datEr       =   XLSX.readdata(NOMBRE_ARCHIVO_DATOS,"ERV!D6:D29") # pw5
    ...
    @constraint(m,flujoTx[j=1:nT],fTx[:,j]/sB .== Y*A'*theta[:,j])
...
result1 = fcn_UC_model(;datGen=datGen,datDem=datDem,
datLineas=datLineas,NOMBRE_ARCHIVO_DATOS=NOMBRE_ARCHIVO_DATOS,
TIPO_SOLVER=TIPO_SOLVER, TIPO_SERIE_DEMANDA=TIPO_SERIE_DEMANDA, 
TIPO_SERIE_ERV=1)

result2 = fcn_UC_model(;datGen=datGen,datDem=datDem,
datLineas=datLineas,NOMBRE_ARCHIVO_DATOS=NOMBRE_ARCHIVO_DATOS,
TIPO_SOLVER=TIPO_SOLVER, TIPO_SERIE_DEMANDA=TIPO_SERIE_DEMANDA, 
TIPO_SERIE_ERV=2)

result3 = fcn_UC_model(;datGen=datGen,datDem=datDem,
datLineas=datLineas,NOMBRE_ARCHIVO_DATOS=NOMBRE_ARCHIVO_DATOS,
TIPO_SOLVER=TIPO_SOLVER, TIPO_SERIE_DEMANDA=TIPO_SERIE_DEMANDA, 
TIPO_SERIE_ERV=3)
...
function fcn_pega_result(result,xf)
...
fcn_pega_result(;NOMBRE_ARCHIVO_DATOS=NOMBRE_ARCHIVO_DATOS,result=result1,)
###############################################################
# Algunas gráficas
###############################################################
pd     =  datDem[:,2:end] # Demanda en barra 3
...
plt=areaplot(x, y, 
    ...
    label = ["pg1" "pg2" "pg3" "pg4" "pg5"],     # Etiqueta de los datos (aparecerá en la leyenda)
    linewidth=0,
    #serie =["y4"]
    #legend = :bottomright)  # Posición de la leyenda
    )
